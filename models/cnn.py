...
class CNNModel(nn.Module):
    ...
    def __init__(self, width, height, outfeats, channel=[], kernel_size=[], maxpool_kernel_size=[], \
                    nonlinear='relu', batchnorm=False, dropout=0.5, device=None):
        super(CNNModel,self).__init__()
        assert type(channel)==list and type(kernel_size) == list
        ...
        if self.nonlinear == 'relu':
            ...
            self.nonlinear = nn.ReLU()
        if maxpool_kernel_size == [] or maxpool_kernel_size is None:
        ...
        for idx in range(self.num_layers):
            ...
            pad_size = int((kernel_size[idx]-1)/2)
            # seldom use dropout in Conv Layer
            conv_layer[idx] = nn.Sequential(
                nn.Conv2d(in_channel, channel[idx], kernel_size[idx], padding=pad_size, stride=1, dilation=1),
            ...
            assert width % maxpool_kernel_size[idx] == 0
            width = int(width/maxpool_kernel_size[idx])
        self.conv_layer = nn.ModuleList(conv_layer)
        self.output_layer = nn.Linear(in_channel*width*width, self.num_classes)
    ...
    def init_weight(self, weight_scale=0.2):
        ...
        torch.nn.init.xavier_uniform_(self.output_layer.weight, weight_scale)
